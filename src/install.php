...
class zeroInstall
{
    ...
    public function composer($args)
    {
        ...
        //--------Extracting composer library
        echo '<br/>AUTOLOAD : '.EXTRACT_DIRECTORY.'/vendor/autoload.php'; //die('<hr/>['.__LINE__.']['.__FILE__.']');
        if (true == \file_exists(EXTRACT_DIRECTORY.'/vendor/autoload.php')) {
            echo "<hr/>Extracted autoload already exists. Skipping phar extraction as presumably it's already extracted.\n";
        } else {
    ...
    {
        if ('public_html' != \basename(__DIR__)) {
    ...
    {
        $pos = \mb_strrpos($text, $str_find);
        $text_before = \mb_substr($text, 0, $pos + \mb_strlen($str_find));
        $text_after = \mb_substr($text, $pos + \mb_strlen($str_find));
        $pos = \mb_strrpos($text, $str_add);
        if (false === $pos) {
    ...
    public function step3()
    {
        ...
        $pos = \mb_strrpos($index_content, $str);
...
';
    ...
    {
        $config_dir = '../laravel/config/'.\mb_strtolower(\str_replace('www.', '', $_SERVER['SERVER_NAME'])).'';
    ...
    public function curlAuthDownload($url, $destination, $user, $pwd)
    {
        try {
