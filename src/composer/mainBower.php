...
function command_array(){  
	...
	*/
	...
	if(isset($_POST['package']) && strlen(trim($_POST['package']))>3 ){
		$args['package']=trim($_POST['package']);
		$args['--save']=true;
	}
	...
	//$args= ['command' => 'install','package'=> 'jqueryui-touch-punch','--save'=>true];
	//$args= ['command' => 'list'];
	
	$input = new Symfony\Component\Console\Input\ArrayInput($args);
	$output = new Symfony\Component\Console\Output\StreamOutput(tmpfile());
	...
	echo '<pre>['.chr(13);print_r($content);echo chr(13).']</pre>';
